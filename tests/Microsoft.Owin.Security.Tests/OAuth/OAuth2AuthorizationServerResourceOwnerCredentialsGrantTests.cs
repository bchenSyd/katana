...
namespace Microsoft.Owin.Security.Tests.OAuth
{
    public class OAuth2AuthorizationServerResourceOwnerCredentialsGrantTests
    {
        ...
        /// <summary>
        /// !!!bchen: entry point. run this test first
        /// </summary>
        [Fact]
        public async Task ResourceOwnerCanSucceedWithoutClientId()
        {
            ...
            {
                //openID scenario
                //OnValidateClientAuthentication: application level authentication; e.g. is client_id/client_secret valid?
                s.Provider.OnValidateClientAuthentication = ctx =>
                {
                    //since we are not using client_id, we always return valid
                    ctx.Validated();
                    return Task.FromResult(0);
                };
                //OnValidateClientAuthentication:
                //here we validate username and password
